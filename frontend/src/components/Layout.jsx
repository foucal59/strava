          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm
                         bg-dark-700 hover:bg-dark-600 transition-colors disabled:opacity-50"
            >
            ...
            {athlete && (
              <div className="flex items-center gap-2">
                {athlete.profile_pic && (
                  <img src={athlete.profile_pic} alt="" className="w-7 h-7 rounded-full" />
                )}
      ...
      <nav className="md:hidden flex items-center gap-1 px-4 py-2 border-b border-dark-600 bg-dark-800 overflow-x-auto">
        {navItems.map(({ to, icon: Icon, label }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `nav-link flex items-center gap-1.5 whitespace-nowrap ${isActive ? 'active' : ''}`
            }
          >
            <Icon size={14} />
            {label}
